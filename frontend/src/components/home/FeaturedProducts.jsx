  ...
  return (
    <section style={{ padding: '8rem 0', background: 'var(--cream)', direction: isAr ? 'rtl' : 'ltr' }}>
      <div className="container-luxe">
        ...
        >
          <div>
            <span style={{ ...MONO, fontSize: '0.65rem', color: 'var(--terracotta)', display: 'block', marginBottom: '1rem' }}>
              {isAr ? '— اختيار الشهر' : lang === 'fr' ? '— Sélection du mois' : '— Monthly Selection'}
            </span>
            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2.5rem, 6vw, 5rem)', fontWeight: 300, lineHeight: 1, letterSpacing: '-0.03em', color: 'var(--charcoal)', margin: 0 }}>
              {isAr ? (
                ...
                <>Our <span style={{ fontStyle: 'italic' }}>top picks.</span></>
              )}
            </h2>
          </div>
          <Link to="/shop" style={{ ...MONO, fontSize: '0.65rem', color: 'var(--charcoal)', borderBottom: '1px solid var(--charcoal)', paddingBottom: '3px', whiteSpace: 'nowrap' }}
            ...
            {isAr ? 'عرض الكل ←' : lang === 'fr' ? 'Tout voir →' : 'View all →'}
          </Link>
        </motion.div>
        ...
        {/* Stats ticker */}
        <div className="stats-ticker" style={{ marginTop: '5rem', borderTop: '1px solid var(--charcoal)', borderBottom: '1px solid var(--line)', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
          {[
            ['130', isAr ? 'عطر' : lang === 'fr' ? 'Parfums' : 'Fragrances'],
            ['10 ml', isAr ? 'حجم' : 'Format'],
            ['48 h', isAr ? 'توصيل' : lang === 'fr' ? 'Livraison' : 'Delivery'],
            ['100%', isAr ? 'أصلي' : lang === 'fr' ? 'Authentique' : 'Authentic'],
          ].map(([big, small], i) => (
